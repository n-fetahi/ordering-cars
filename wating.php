...
<?php
				include 'connect.php';
					...
					try{
						...
						$sql = "SELECT * FROM cars where car_state=0 ";
						$stmt = $conn->query($sql);
						$data = $stmt->fetchAll();
						$count = $stmt->rowCount();
              ...
              $data2 = $stmt2->fetch();
							...
							echo "
              <div class='section'>
              <img src='$row[car_img]' alt='صورة 1'>
              <h2>  $data2[username] </h2>
              <p>رقم اللوحة : $row[car_num]</p>
              <a href='wating_det.php?id=$row[ID]'><button class='btn'>عرض المزيد</button></a>
            </div>
							";
						}
						if($f == 0){
